...
<%
   ...
   int nextBlock = prevBlock + pageBlock + 1;
	...
	try {
		...
		String sp = "select count(*) as cnt from `boardschema`.`listboard` where";
		...
		String findsql1 = "select * from `boardschema`.`listboard` where";
		String findsql2 = "order by id desc limit ?, ?";
		//String findsql = "select * from `boardschema`.`listboard` where ? = ? order by id desc limit ?, ?";
		if(searchtype.equals("findTitle")){
				...
				<table width="100%" cellpadding="0" cellspacing="0" border="0">
					...
					<tr>
						...
						<td style="text-align: left; padding-left: 40px;"><a
							href="review_view.jsp?idx=<%=idx %>"><%=title %></a></td>
						<td><%=writer %></td>
						<td><%=days %></td>
						<td><%=view %></td>
						<td><%=review %></td>
					...
					<tr>
						<td style="background-opacity: 0.2;">
							<%
								...
								if(pg>pageSize){%> <a href="find_info_list.jsp?pg=<%=1 %>&searchtype=<%=searchtype%>&searchtext=<%=searchtext%>">처음으로</a>
							<a href="find_info_list.jsp?pg=<%=startPage-1 %>&searchtype=<%=searchtype%>&searchtext=<%=searchtext%>">이전</a> <% } %>

							<%
								for(int i = startPage; i <= endPage; i++){
									if(i==pg){
									%> <%=i %> <%}else{%> <a href="find_info_list.jsp?pg=<%=i %>&searchtype=<%=searchtype%>&searchtext=<%=searchtext%>"><%=i %></a>
							<% }
								}%> <%if(endPage < totalPage){ %> <a
							href="find_info_list.jsp?pg=<%=endPage+1 %>&searchtype=<%=searchtype%>&searchtext=<%=searchtext%>">다음</a> <a
							href="find_info_list.jsp?pg=<%=totalPage %>&searchtype=<%=searchtype%>&searchtext=<%=searchtext%>">맨뒤로</a> <%} %>
						</td>
					</tr>
				</table>
				<table>
					<form name=form2 action="find_info_list.jsp?pg=<%=pg%>">
					<tr>
						<td width=20%;></td>
						...
						onclick="pleaseLogin();"
						<%}else{ %>
						onclick="window.location='info_write.jsp'" 
						<%} %>
