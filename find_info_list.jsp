...
<%
   ...
   int nextBlock = prevBlock + pageBlock + 1;
	...
	try {
		...
		String sp = "select count(*) as cnt from `boardschema`.`infolist` where";
		...
		String findsql1 = "select * from `boardschema`.`infolist` where";
		String findsql2 = "order by id desc limit ?, ?";
		//String findsql = "select * from `boardschema`.`infolist` where ? = ? order by id desc limit ?, ?";
		if(searchtype.equals("findTitle")){
