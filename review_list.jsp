...
<%
	...
	try {
		...
		String sp = "select count(*) from listboard";			//게시글 갯수
		String sql = "select * from `boardschema`.`listboard` order by id desc limit ?, ?";		//페이지당 표기할 숫자
	...
	<div id="main">
		<div class="inner">
			...
			<!-- 상단 메뉴 끝 -->
					...
					<tr>
						<td style="background-opacity: 0.2;">
							<%
								...
								if (pg > pageSize) {
							%> <a href="review_list.jsp?pg=<%=1%>">처음으로</a> <a
							href="review_list.jsp?pg=<%=startPage - 1%>">이전</a> <%
							 	}
							 %> <%
							 ...
							 %> <a href="review_list.jsp?pg=<%=i%>"><%=i%></a> <%
							 	}
							 	}
							 %> <%
							 	if (endPage < totalPage) {
							 %> <a href="review_list.jsp?pg=<%=endPage + 1%>">다음</a> <a
														href="review_list.jsp?pg=<%=totalPage%>">맨뒤로</a> <%
							 	}
							 %>
						</td>
					</tr>
				...
				<table>
					<form name=form2 action="find_review_list.jsp">
						<tr>
							<td width=20%;></td>
							...
							<td><input type="button" value="글쓰기" class="style2"
								...
								onclick="window.location='review_write.jsp'" <%}%> /></td>
						</tr>
					</form>
				</table>
